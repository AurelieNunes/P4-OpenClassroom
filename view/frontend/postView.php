    ...
    <?php 
        if (!empty($_SESSION)) {
            if ($comment['author'] !== $_SESSION['pseudo']) {
                echo '<p class="report text-center"><a href="index.php?action=report&amp;id=' . htmlspecialchars($comment['post_id']) . '&amp;comment-id=' . htmlspecialchars($comment['id']) . '">Signaler</a></p>';
            }
        }
    }
        ?>
